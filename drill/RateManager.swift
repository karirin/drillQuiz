...
//    }
    ...
    func fetchSansuQuizData(userId: String, completion: @escaping ([QuizSansuLevel: QuizData]) -> Void) {
            let ratesRef = db.child("rates").child(userId)

            ratesRef.observeSingleEvent(of: .value, with: { snapshot in
                var quizData = [QuizSansuLevel: QuizData]()
                for level in QuizSansuLevel.allCases {
                    if let levelData = snapshot.childSnapshot(forPath: level.description).value as? [String: Int],
                       let correct = levelData["correct"],
                       let answer = levelData["answer"] {
//                        print("test:\(levelData)")
    ...
    func fetchKokugoQuizData(userId: String, completion: @escaping ([QuizKokugoLevel: QuizData]) -> Void) {
            let ratesRef = db.child("rates").child(userId)

            ratesRef.observeSingleEvent(of: .value, with: { snapshot in
                var quizData = [QuizKokugoLevel: QuizData]()

                for level in QuizKokugoLevel.allCases {
                    if let levelData = snapshot.childSnapshot(forPath: level.description).value as? [String: Int],
                       let correct = levelData["correct"],
                       let answer = levelData["answer"] {
                        quizData[level] = QuizData(answer: answer, correct: correct)
    ...
    func fetchShakaiQuizData(userId: String, completion: @escaping ([QuizShakaiLevel: QuizData]) -> Void) {
            let ratesRef = db.child("rates").child(userId)

            ratesRef.observeSingleEvent(of: .value, with: { snapshot in
                var quizData = [QuizShakaiLevel: QuizData]()

                for level in QuizShakaiLevel.allCases {
                    if let levelData = snapshot.childSnapshot(forPath: level.description).value as? [String: Int],
                       let correct = levelData["correct"],
                       let answer = levelData["answer"] {
                        quizData[level] = QuizData(answer: answer, correct: correct)
    ...
    func fetchRikaQuizData(userId: String, completion: @escaping ([QuizRikaLevel: QuizData]) -> Void) {
            let ratesRef = db.child("rates").child(userId)

            ratesRef.observeSingleEvent(of: .value, with: { snapshot in
                var quizData = [QuizRikaLevel: QuizData]()

                for level in QuizRikaLevel.allCases {
                    if let levelData = snapshot.childSnapshot(forPath: level.description).value as? [String: Int],
                       let correct = levelData["correct"],
                       let answer = levelData["answer"] {
                        quizData[level] = QuizData(answer: answer, correct: correct)
